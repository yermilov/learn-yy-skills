...
| **home-it** | Practical playbooks for the tech in your home. `check-network` diagnoses both DNS-specific failures and slow networks: it compares system/router/public resolvers, checks negative caches/upstreams/filters, measures throughput/latency/loss/link quality where relevant, inspects the router UI, and verifies one repair at a time. |
